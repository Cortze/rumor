...
func (c *TopicEventsCmd) Run(ctx context.Context, args ...string) (err error) {
	...
	top, ok := c.GossipState.Topics.Load(c.TopicState.Eth2TopicName)
	if !ok {
	...
	go func() {
		c.Log.Infof("Started listening for peer join/leave events for topic %s", c.TopicState.Eth2TopicName)
		for {
			ev, err := evHandler.NextPeerEvent(ctx)
			if err != nil {
				c.Log.Infof("Stopped listening for peer join/leave events for topic %s", c.TopicState.Eth2TopicName)
				return
			}
			switch ev.Type {
			case pubsub.PeerJoin:
				c.Log.WithFields(logrus.Fields{"peer_id": ev.Peer, "topic": c.TopicState.Eth2TopicName}).Info("topic joined")
			case pubsub.PeerLeave:
				c.Log.WithFields(logrus.Fields{"peer_id": ev.Peer, "topic": c.TopicState.Eth2TopicName}).Info("topic left")
			}
		}
	}()
